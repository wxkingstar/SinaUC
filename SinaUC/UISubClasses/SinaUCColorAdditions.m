...
@implementation NSDictionary (SinaUCColorAdditions_RGBTxtFiles)
...
@implementation NSColor (SinaUCColorAdditions_RGBTxtFiles)
...
@implementation NSColor (SinaUCColorAdditions_Comparison)
...
@implementation NSColor (SinaUCColorAdditions_DarknessAndContrast)
...
@implementation NSColor (SinaUCColorAdditions_HLS)
...
@implementation NSColor (SinaUCColorAdditions_RepresentingColors)
...
@implementation NSString (SinaUCColorAdditions_RepresentingColors)
...
@implementation NSColor (SinaUCColorAdditions_RandomColor)
...
@implementation NSColor (SinaUCColorAdditions_HTMLSVGCSSColors)
...
+ (id)colorWithHTMLString:(NSString *)str defaultColor:(NSColor *)defaultColor
{
	...
	if ([str hasPrefix:@"rgb"]) {
		...
		{
			NSLog(@"+[NSColor(SinaUCColorAdditions) colorWithHTMLString:] called with unrecognised color function (str is %@); returning %@", str, defaultColor);
			return defaultColor;
		}
		leftParIndex++;
		...
		if ([colorComponents count] < 3 || [colorComponents count] > 4) {
			NSLog(@"+[NSColor(SinaUCColorAdditions) colorWithHTMLString:] called with a color function with the wrong number of arguments (str is %@); returning %@", str, defaultColor);
			return defaultColor;
		}
	...
	if ((!strLength) || ([str characterAtIndex:0] != '#')) {
		...
		if (!colorValue) {
#if COLOR_DEBUG
			NSLog(@"+[NSColor(SinaUCColorAdditions) colorWithHTMLString:] called with unrecognised color name (str is %@); returning %@", str, defaultColor);
#endif
			return defaultColor;
		}
	}
...
@implementation NSColor (SinaUCColorAdditions_ObjectColor)
