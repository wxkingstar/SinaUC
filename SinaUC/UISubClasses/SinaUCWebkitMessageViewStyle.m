//
//  SinaUCWebkitMessageViewStyle.m
//  SinaUC
//
...
#import "SinaUCWebkitMessageViewStyle.h"
...
@interface SinaUCWebkitMessageViewStyle ()
...
@implementation SinaUCWebkitMessageViewStyle
...
- (NSString *)baseTemplateForChat
{
	...
	return [self fillKeywordsForBaseTemplate:templateHTML];
}

- (NSString *)templateForContent:(SinaUCMessage *)message similar:(BOOL)contentIsSimilar
{
	NSString	*template;
	...
	if ([content isKindOfClass:[AIContentMessage class]]) {
		...
		if (range.location != NSNotFound) {
			...
			if (replacedStatusPhrase) {
				...
				messageString = [SinaUCHTMLDecoder encodeHTML:[content message]
												  headers:NO
												 fontTags:NO
									   includingColorTags:NO
											closeFontTags:YES
...
- (NSMutableString *)fillKeywordsForBaseTemplate:(NSMutableString *)inString
{
	NSRange	range;
...
/*- (NSString *)iconPathForFileTransfer:(SinaUCFileTransfer *)inObject
{
